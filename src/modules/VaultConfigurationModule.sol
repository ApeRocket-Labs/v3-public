...
contract VaultConfigurationModule is Auth {
    ...
    /// @notice Emitted when the default target float percentage is updated.
    /// @param newDefaultWithdrawalFeePercent The new default target float percentage.
    event DefaultWithdrawalFeePercentUpdated(uint256 newDefaultWithdrawalFeePercent);
    ...
    /// @notice The default withdrawal fee percentage for Vaults.
    uint256 public defaultWithdrawalFeePercent;
    ...
    /// @notice Sets the default withdrawal fee percentage for Vaults.
    /// @param newDefaultWithdrawalFee The new default target float percentage to set.
    function setDefaultWithdrawalFee(uint256 newDefaultWithdrawalFee) external requiresAuth {
        // Update the default target float percentage.
        defaultWithdrawalFeePercent = newDefaultWithdrawalFee;

        emit DefaultWithdrawalFeePercentUpdated(newDefaultWithdrawalFee);
    }
    ...
    /// @notice Emitted when a Vault has its withdrawal fee percentage set/updated.
    /// @param vault The Vault that had its custom withdrawal fee percentage set/updated.
    /// @param newCustomWithdrawalFeePercent The new custom withdrawal fee percentage for the Vault.
    event CustomWithdrawalFeePercentUpdated(Vault indexed vault, uint256 newCustomWithdrawalFeePercent);
    ...
    /// @notice Maps Vaults to their custom withdrawal fee percentage.
    /// @dev Will be 0 if there is no custom target float percentage for the Vault.
    mapping(Vault => uint256) public getVaultCustomWithdrawalFeePercent;
    ...
    /// @notice Sets the custom target float percentage for the Vault.
    /// @param vault The Vault to set the custom target float percentage for.
    /// @param customWithdrawalFeePercent The new custom target float percentage to set.
    function setVaultCustomWithdrawalFeePercent(Vault vault, uint256 customWithdrawalFeePercent) external requiresAuth {
        // Update the Vault's custom target float percentage.
        getVaultCustomWithdrawalFeePercent[vault] = customWithdrawalFeePercent;

        emit CustomWithdrawalFeePercentUpdated(vault, customWithdrawalFeePercent);
    }
    ...
    /// float percentage or the default target float percentage if a custom percentage is not set.
    /// @param vault The Vault to sync the target float percentage for.
    function syncWithdrawalFeePercent(Vault vault) external {
        // Get the Vault's custom target float percentage.
        uint256 customWithdrawalFeePercent = getVaultCustomWithdrawalFeePercent[vault];

        // Set the Vault's target float percentage to the custom target float percentage
        // or the default target float percentage if a custom percentage isn't set.
        vault.setWithdrawalFeePercent(
            customWithdrawalFeePercent == 0 ? defaultWithdrawalFeePercent : customWithdrawalFeePercent
        );
    }
}
